...
export function NavbarCentered({ className }: { className?: string }) {
  return (
    <nav
      className={cn(
        "fixed top-4 left-1/2 -translate-x-1/2 z-50 w-[95%] max-w-5xl rounded-full border border-border/40 bg-background/80 backdrop-blur-md shadow-sm",
        className,
      )}
    >
      <div className="px-6 h-14 flex items-center justify-between">
        ...
        <div className="hidden md:flex items-center justify-center gap-1 absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2">
          <Link
            href="#"
            className="px-4 py-2 rounded-full hover:bg-accent/50 transition-colors"
          >
